      ...
      <div className="w-full h-screen bg-white">
        ...
        <section className='container p-4'>
          <div className='flex justify-between place-items-center'>
            <h1 className='text-3xl font-bold'>Afif Mart</h1>
          ...
          <div className='flex overflow-x-auto mb-3'>
            <span key={`category-00`} className="inline-flex mx-1 items-center rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-700/10">SEMUA</span>
            {
            categories.data && categories.data.map((item, itme) => (
              <span key={`category-${Index}`} className="inline-flex mx-1 items-center rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-700/10">{item.name}</span>
            ))
          } </div>
            ...
            products.data && products.data.map((item, index) => (
              <Link key={
                ...
                <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
                  <img src={item.image ?? `https://tailwindui.com/img/ecommerce-images/category-page-04-image-card-01.jpg`} alt="Tall slender porcelain bottle with natural clay textured body and cork stopper." className="h-full w-full object-cover object-center group-hover:opacity-75"/>
                </div>
